...
import "../styles/AdminCustumer.scss";
...
function AdminCustumer() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3030/custumers")
      .then((res) => setData(res.data))
      .catch((err) => console.log(err));
  });
  const handleSubmit = (event) => {
    event.preventDefault();
    axios.post("http://localhost:3030/custumers", inputData).then((res) => {
  ...
  function handleDelete(id) {
    ...
    if (confirm) {
      axios.delete("http://localhost:3030/custumers/" + id).then((res) => {
